...
5. [Issues Encountered During Deployment and Solutions](#issues-encountered-during-deployment-and-solutions)
...
flowchart TB
    ...
    subgraph OnPrem["On-Premises Network (<your-network-cidr>)"]
        subgraph SAPGW["SAP Gateway (<your-sap-internal-ip>:44300)"]
            SO["Sales Order"]
            CU["Customer"]
            MA["Material"]
            FL["Flight"]
...
- **IP**: Uses internal IP `<your-sap-internal-ip>` (external IP not accessible)
...
#### 4. GlobalGemini Model Class
- **Reason**: Agent Engine overrides `GOOGLE_CLOUD_LOCATION` to the deployment region, but Gemini 3 models require the global endpoint
- **Solution**: Custom `GlobalGemini` subclass of `Gemini` in `agent.py` that overrides `api_client` property to explicitly use `location="global"`
- **Reference**: [google/adk-python#3628](https://github.com/google/adk-python/issues/3628#issuecomment-3595215761)
...
Before deploying to production, review and address the following security items:

### SSL Verification

The `verify_ssl` setting in `SAPConnectionConfig` defaults to `False`, which disables TLS certificate validation. This is acceptable for development with self-signed certificates but must be changed for production.

- Set `verify_ssl=True` in the SAP connection configuration for production deployments
- Ensure valid TLS certificates are installed on the SAP Gateway server
- If using Private Service Connect, verify that certificate chain trust is properly configured

### Credential and Logging Hygiene

- **Remove debug print statements**: The current codebase contains `print()` statements in `agent.py` that output Secret Manager key names, SAP host IP addresses, and GCP project IDs to Cloud Logging. Remove or replace these with structured logging (using the `logging` module or `structlog`) before production deployment.
- **Avoid logging credentials**: Do not log SAP usernames, passwords, or authentication tokens at any log level.
- **Use environment variables for passwords**: When running locally, use `.env` files or environment variables rather than passing passwords as CLI arguments (which are visible via process listing tools such as `ps`).

### Input Validation

- **Sanitize OData filter inputs**: The `$filter` parameter passed to SAP OData queries is not currently sanitized, which could allow OData injection. Integrate the validation functions from `utils/validators.py` into the query pipeline before accepting user-provided filter expressions in production.

### Secret Manager Access

- Ensure the Secret Manager secret (`sap-credentials`) is restricted to the minimum required service accounts.
- Rotate SAP credentials periodically and update the secret version accordingly.
...
**Cause**:
- External IP (`<your-sap-external-ip>`) configured in Secret Manager
...
|-------------|------------|---------|
| Local Development | <your-sap-external-ip> (External) | Access via internet |
| Agent Engine | <your-sap-internal-ip> (Internal) | Access via PSC |
...
> **Note on dependency management**: The `requirements` list in `deploy_agent_engine.py` is maintained separately from the project's `pyproject.toml`. These two dependency lists may drift out of sync over time. When adding or updating dependencies, ensure both files are updated consistently to avoid deployment failures or missing packages.
...
```python
MODEL_NAME = get_model_name()  # Default: "gemini-3-pro-preview", override via SAP_AGENT_MODEL env var
MODEL = get_model()            # GlobalGemini instance (forces global endpoint for Gemini 3 models)
...
| SAP Host (Internal) | <your-sap-internal-ip>:44300 |
