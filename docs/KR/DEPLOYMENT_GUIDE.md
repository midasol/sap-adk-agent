...
- **IP**: 내부 IP `<your-sap-internal-ip>` 사용 (외부 IP 불가)
...
**원인**:
- Secret Manager에 외부 IP (`<your-sap-external-ip>`) 설정됨
...
| 환경 | IP 주소 | 용도 |
|------|---------|------|
| 로컬 개발 | <your-sap-external-ip> (외부) | 인터넷 통한 접근 |
| Agent Engine | <your-sap-internal-ip> (내부) | PSC 통한 접근 |
...
```python
MODEL_NAME = get_model_name()  # 기본값: "gemini-3-pro-preview", SAP_AGENT_MODEL 환경변수로 변경 가능
MODEL = get_model()            # GlobalGemini 인스턴스 (Gemini 3 모델의 글로벌 엔드포인트 강제 적용)
