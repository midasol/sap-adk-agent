...
> **Note**: Replace `[your-sap-internal-ip]` with the actual internal IP address of your SAP Gateway server (e.g., `10.x.x.x` when using PSC).

| Item | Value |
|------|-------|
| SAP Host (Internal) | `[your-sap-internal-ip]` |
| SAP Port | `44300` |
| Model | `gemini-3-pro-preview` (configurable via `SAP_AGENT_MODEL`) |
| Region | `us-central1` |
...
```bash
# Replace placeholder values with your actual SAP environment settings
echo '{
    ...
    subgraph Root["agent-adk-sap-gw/"]
        subgraph SapAgent["sap_agent/"]
            ...
            subgraph GWConnector["sap_gw_connector/"]
                ...
                subgraph Tools["tools/ (unused)"]
                    ToolFiles["base.py, query_tool.py,<br/>entity_tool.py, service_tool.py,<br/>auth_tool.py"]
                end
            end
        end
...
|------|--------|----------|
| SSL Verification | Set `verify_ssl=True` in `SAPConnectionConfig` | Critical |
| Debug Statements | Remove all `print()` statements from `agent.py`; replace with structured logging | Critical |
| OData Filter Validation | Integrate `utils/validators.py` to sanitize `$filter` inputs | High |
| Credential Handling | Use environment variables or Secret Manager for credentials; do not pass passwords as CLI arguments | High |
| Log Sanitization | Ensure Secret Manager key names, SAP host IPs, and project IDs are not logged | High |
| Secret Rotation | Establish a schedule for rotating SAP credentials in Secret Manager | Medium |
